...
def update_product(db: Session, id: int, data: ProductUpdate) -> Product:
    ...
    if "category_id" in update_data and not db.get(Category, update_data["category_id"]):
        raise HTTPException(status_code=400, detail="Invalid category_id mapping target")
    if "supplier_id" in update_data and not db.get(Supplier, update_data["supplier_id"]):
        raise HTTPException(status_code=400, detail="Invalid supplier_id mapping target")
