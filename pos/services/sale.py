...
def get_sale(db: Session, id: int):
    ...
    if not sale:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sale invoice record not found")
    return sale
...
def create_sale(db: Session, data: SaleCreate, current_user: User):
    if data.customer_id and not customer_repository.get(db, data.customer_id):
        raise HTTPException(status_code=400, detail="Invalid customer_id profile connection")
