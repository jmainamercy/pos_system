...
def issue_receipt(db: Session, sale_id: int):
    ...
    if not sale:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Sale invoice record missing")
...
def get_receipt(db: Session, id: int):
    ...
    if not receipt:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Requested receipt not found")
    return receipt
